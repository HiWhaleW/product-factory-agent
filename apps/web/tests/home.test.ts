...
describe("home project projections", () => {
  ...
  it("opens onboarding before account creation and stops after completion", () => {
    const loggedOut = { auth_enforced: true, authenticated: false };
    const loggedIn = { auth_enforced: true, authenticated: true };

    expect(onboardingAllowed(loggedOut)).toBe(true);
    expect(shouldAutoOpenOnboarding(loggedOut, "/", false)).toBe(true);
    expect(shouldAutoOpenOnboarding(loggedOut, "/", true)).toBe(false);
    expect(shouldAutoOpenOnboarding(loggedIn, "/", false)).toBe(false);
  });

  it("tracks the pre-account deployment onboarding", () => {
    expect(onboardingStorageKey()).toBe("product-factory:onboarding:v3:pre-auth");
  });
  ...
  it("guides first-time users to configure their own API key", () => {
    ...
    expect(onboardingSteps[1].body).toContain("项目、产物和 API 配置");
  });
});
