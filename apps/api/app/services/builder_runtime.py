    ...
    def _builder_prompt(
        ...
        if stage == "development_frontend":
            stage_instructions = (
                ...
                "本阶段只实现销售复盘 Agent 自身前端，不修改产品工厂前端。"
                "使用冻结的 Next.js 16.3.1、React 19.2.8、Tailwind 4.3.3，连接当前工作区"
                "已核验的真实 FastAPI API；不得用 mock 数据放行。实现材料上传、状态、结论编辑、"
                "行动项编辑、导出和历史回看，完成 lint、typecheck、测试和 production build。"
            )
        else:
            stage_instructions = (
                f"先读取 `{context_path}/` 下全部已批准技术材料和 `CONTEXT.md`。"
                "本阶段只完成后端，不开发前端。创建可独立运行的销售复盘 Agent "
                "FastAPI/PostgreSQL/Alembic 后端，完整实现 API Contract，运行真实单元和集成测试。"
            )
        return (
            f"{frozen_prompt}\n\n"
            "# 本次已批准执行上下文\n\n"
