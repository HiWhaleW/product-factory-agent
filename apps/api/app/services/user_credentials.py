...
class UserSecretStore:
    ...
    def __init__(self, root: Path, provider: str = MODEL_PROVIDER) -> None:
        self.root = root.resolve(strict=False)
        if provider not in {MODEL_PROVIDER, RESEARCH_PROVIDER}:
            raise UserCredentialError("USER_SECRET_PROVIDER_INVALID", "不支持的服务类型。")
    ...
    def _path(self, user_id: str) -> Path:
        return self._directory(user_id) / f"{self.provider}.key"

    def secret_ref(self, user_id: str) -> str:
        identity = hashlib.sha256(user_id.encode()).hexdigest()
        return f"user-secret://{identity}/{self.provider}"
    ...
    def write(self, user_id: str, api_key: str) -> None:
        ...
        temporary = directory / f".{self.provider}.{uuid4().hex}.tmp"
...
def credential_record(
    session: Session, user_id: str, provider: str = MODEL_PROVIDER
...
def validate_research_config(provider_name: str, base_url: str) -> tuple[str, str]:
    provider_name = provider_name.strip()
    base_url = base_url.strip().rstrip("/")
    parsed = urlparse(base_url)
    if not provider_name:
        raise UserCredentialError(
            "USER_RESEARCH_CONFIG_INVALID", "API 厂商不能为空。"
        )
    if (
        parsed.scheme != "https"
        or not parsed.hostname
        ...
        raise UserCredentialError(
            "USER_RESEARCH_CONFIG_INVALID",
            "服务地址必须是公开可访问的 HTTPS 地址。",
        )
    hostname = parsed.hostname.lower()
    if hostname == "localhost" or hostname.endswith(".localhost"):
        raise UserCredentialError(
            "USER_RESEARCH_CONFIG_INVALID", "服务地址不能指向本机或内网。"
        )
    try:
        address = ipaddress.ip_address(hostname)
    except ValueError:
...
def research_runtime_supported(provider_name: str | None, base_url: str | None) -> bool:
    if not provider_name or not base_url:
        return False
    normalized = provider_name.strip().lower()
    parsed = urlparse(base_url)
    return normalized in {"bocha", "博查"} and parsed.hostname == "api.bochaai.com"


def research_credential_status(
    session: Session, *, settings: Settings, user_id: str
) -> CredentialStatus:
    record = credential_record(session, user_id, RESEARCH_PROVIDER)
    available = False
    if record is not None:
        assert settings.USER_SECRET_ROOT is not None
        available = (
            UserSecretStore(settings.USER_SECRET_ROOT, RESEARCH_PROVIDER).read(user_id)
            is not None
        )
    return CredentialStatus(
        configured=available,
...
def save_research_credential(
    ...
    *,
    ...
    api_key = validate_api_key(api_key)
    provider_name, base_url = validate_research_config(provider_name, base_url)
    assert settings.USER_SECRET_ROOT is not None
    store = UserSecretStore(settings.USER_SECRET_ROOT, RESEARCH_PROVIDER)
    previous = store.read(user_id)
    store.write(user_id, api_key)
    try:
        record = credential_record(session, user_id, RESEARCH_PROVIDER)
...
def resolve_research_credential(
    session: Session, *, settings: Settings, user_id: str
) -> ResearchCredential:
    record = credential_record(session, user_id, RESEARCH_PROVIDER)
    if record is None:
        raise UserCredentialError(
            "USER_RESEARCH_API_KEY_REQUIRED",
            "请先前往设置页添加你的网络搜索 API。",
        )
    assert settings.USER_SECRET_ROOT is not None
    value = UserSecretStore(settings.USER_SECRET_ROOT, RESEARCH_PROVIDER).read(user_id)
    if not value:
        raise UserCredentialError(
            "USER_RESEARCH_API_KEY_UNAVAILABLE",
            "已保存的网络搜索 API Key 暂时不可用，请在设置中重新添加。",
        )
    return ResearchCredential(
        provider_name=record.provider_name,
        base_url=validate_research_config(record.provider_name, record.base_url)[1],
        api_key=value,
    )
