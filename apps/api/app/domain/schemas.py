...
class ProjectCreate(ApiModel):
    ...
    initial_goal: str | None = Field(default=None, min_length=1, max_length=50_000)
    initial_message_id: str | None = Field(default=None, min_length=1, max_length=100)

    @model_validator(mode="after")
    def initial_goal_requires_message_id(self) -> "ProjectCreate":
        if (self.initial_goal is None) != (self.initial_message_id is None):
            raise ValueError("initial_goal and initial_message_id must be provided together")
        return self
...
class ResearchCredentialRead(ApiModel):
