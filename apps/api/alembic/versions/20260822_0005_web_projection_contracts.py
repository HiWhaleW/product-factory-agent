...
SALES_PROJECT_ID = "2a3c38e1-9704-4f83-a096-84cb5a5025e7"
SALES_G1_ID = "cec40b01-ba61-494b-a057-b2f5c74173f1"
SALES_REVIEW_ID = "d383d981-e599-4daf-8dd1-836e9e360f93"
...
def upgrade() -> None:
    ...
    op.execute(
        sa.text(
            ...
            WHERE id = :project_id
            """
        ).bindparams(project_id=SALES_PROJECT_ID)
    )
    op.execute(
        sa.text(
            "UPDATE gates SET known_issues = CAST(:issues AS json) WHERE id = :gate_id"
        ).bindparams(issues=SALES_KNOWN_ISSUES, gate_id=SALES_G1_ID)
    )
    op.execute(
        sa.text(
            """
            ...
            WHERE id = :review_id
            """
        ).bindparams(issues=SALES_KNOWN_ISSUES, review_id=SALES_REVIEW_ID)
    )
