...
本规格把 [Interaction-Spec.md](./Interaction-Spec.md) 的交互定义拆成可实现组件、事件投影、状态矩阵、响应式规则、每日任务和退出证据。D3 已存在明确标记 mock 的基本渲染壳，但它当前只是静态项目事件列表和线性节点，不符合权威“团队群聊 + 产物画布”基线；真实 API、模型、正式 React Flow DAG 和交互符合性验收均未完成。
