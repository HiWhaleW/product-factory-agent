...
> 实施状态不反向修改本冻结 PRD；当前阶段、双环境和验证结果见 [当前交接](./handoff.html) 与 [双环境运行说明](./environments.html)。
...
- Agent 是主要执行者，不是传统系统里的聊天插件。
- 自然语言是主要入口；Agent 读取版本化 Context，调用受控工具，生成可追溯 Artifact，并接受 Reviewer 与人工 Gate。
- 系统必须支持长任务、暂停恢复、失败修订和“数据与反馈 → 下一轮”的闭环。
- 确定性状态机、权限、预算和审计为 Agent 自主行动提供边界，不由模型自由决定。
- 如果一个功能只增加表单、按钮或固定工作流，却没有增强 Agent 的感知、行动、产物或反馈能力，就不能算 AI Native 能力完成。
