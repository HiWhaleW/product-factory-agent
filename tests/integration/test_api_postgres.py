...
def test_health_uses_live_postgresql() -> None:
    ...
    assert response.json()["model_provider"] == "user_configured"
...
def test_runtime_status_checks_codex_without_exposing_secrets() -> None:
    ...
    assert body["model_provider"] == "user_configured"
...
def test_project_sequential_idempotency_and_input_conflict(namespace: str) -> None:
    ...
    body = {
        ...
        "initial_goal": "Build a real review Agent",
        "initial_message_id": f"{namespace}-initial-goal",
    }
    with TestClient(app) as client:
        ...
        messages = client.get(f"/api/v1/projects/{first.json()['id']}/messages")
        events = client.get(f"/api/v1/projects/{first.json()['id']}/events")
    ...
    assert [item["content"] for item in messages.json()] == ["Build a real review Agent"]
    assert messages.json()[0]["actor_id"] == namespace
