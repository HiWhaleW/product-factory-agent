...
def test_registered_users_only_see_their_own_projects(monkeypatch) -> None:
    namespace = str(uuid4())
    username_a = f"user-a-{namespace}"
    username_b = f"user-b-{namespace}"
    password_a = f"password-a-{namespace}"
    password_b = f"password-b-{namespace}"
    ...
    try:
        with TestClient(app) as client_a, TestClient(app) as client_b:
            login_a = client_a.post(
                ...
                json={"username": username_a, "display_name": "测试用户 A", "password": password_a},
            )
            login_b = client_b.post(
                "/api/v1/auth/register",
                json={"username": username_b, "display_name": "测试用户 B", "password": password_b},
            )
            assert login_a.status_code == 201, login_a.text
            assert login_b.status_code == 201, login_b.text
            user_a = login_a.json()
            user_b = login_b.json()
